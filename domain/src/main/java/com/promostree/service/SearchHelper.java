...
public List<SolrVenue> getOfferInVenue( Venue venue) {
		...
		for (Offer offer : offs) {
			...
			doc.setUpdated_by(venue.getUpdatedBy());
			doc.setUpdated_dt(venue.getUpdatedDate().toString());
			doc.setCreated_by(venue.getCreatedBy());
			doc.setCreated_dt(venue.getCreatedDate().toString());
			// venues.setFs_id(venue.);

			// Address
			doc.setAddress_id(venue.getAddress().getId().toString());
			doc.setCity(venue.getAddress().getCity());
			doc.setCountry(venue.getAddress().getCountry());
			doc.setRegion(venue.getAddress().getLandMark());
			doc.setState(venue.getAddress().getState());

			// Location
			doc.setLocation_id(venue.getAddress().getLocation().getId()
					.toString());
			doc.setLat(new Double(venue.getAddress().getLocation().getLat())
					.toString());
			doc.setLng(new Double(venue.getAddress().getLocation().getLng())
					.toString());
			...
			doc.setCat_name(venue.getCategory().getName());
			// venues.setSub_cat_name(sub_cat_name);

			// offer
			doc.setOffer_id(offer.getId().toString());
			doc.setOffer(offer.getSubject());
			doc.setEnd_dt(offer.getEndingDate().toString());
			doc.setOffer_desc(offer.getDescription());
			doc.setStart_dt(offer.getStartingDate().toString());
	...
	public List<Venue> toDomainVenues(List<SolrVenue> docsWithDiffId) {
		...
		for (String entityId : entityIds) {
			...
			if (!docsWithSameId.isEmpty()) {
				domainVenues.add(fromDocToDomainVenue(docsWithSameId));
			}
	...
	public Venue fromDocToDomainVenue(List<SolrVenue> docsWithSameId) {
		...
		if (!docsWithSameId.isEmpty()) {
			...
			location.setLat(Double.parseDouble(doc.getLat()));
			location.setLng(Double.parseDouble(doc.getLng()));
	...
	//query with searchterm and venues based on nearest distace
	public List<Venue> queriedVenues(String searchTerm,double lat, double lng, int dist ){
		...
		for (SolrVenue doc : solrRepository.findByQueryAnnotation(searchTerm,lat1, lng1)) {
			...
			if (this.distance(lat, lng, Double.parseDouble(lat2),
					Double.parseDouble(lng2), 'k') <= dist) {
				v2.add(doc);
			}
		}
		return toDomainVenues(v2);
	}
